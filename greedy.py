...
def set_layer_and_calculate_greedy(list_network, sys_list, i, output_list, output_cycle = [], verbose=True):
    current_pool = []
    ...
    for i, systolic in enumerate(sys_list): # descent layer
        if i >= len(list_network):
            break
        if list_network[i].systolic is None:
            list_network[i].set_systolic(systolic)
            list_network[i].set_layer_and_current_cycle()
...
def main(args):
    ...
    while return_trigger:
        return_trigger = set_layer_and_calculate_greedy(list_network, sys_list, i, output_list, output_cycle = output_cycles, verbose=True)
